...
module.exports = {
  ...
  async load(contentDir) {
    ...
    if(await fsExtra.pathExists(configPath)) {
      const config = eno.parse(await fsExtra.readFile(configPath, 'utf-8'));
      this.context.contentDir = contentDir;
      this.context.config = {};

      for(let site of config.elements()) {
        this.context.config[site.name] = {
          directory: site.string('directory', { required: true }),
          host: site.string('host', { required: true }),
          user: site.string('user', { required: true })
        };
      }
    }
  },
